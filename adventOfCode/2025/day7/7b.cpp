...
void solve() {
  string s;
  int ans = 0;
  vector<int> beam(142,0), next(142,0);
  cin >> s;
  ...
  vector<vector<int>> dp(142, vector<int>(142,0));
  // dp[i][j] = number of diff ways for spliting...if [i][j] was the source
  // if [i][j] == '^',  dp[i][j] = dp[i+1][j-1] + dp[i+1][j+1]

...
int calc(vector<string>& a, vector<int>& cols, int ind) {
  ...
  for (int i = cols.size() - 1; i >= 0; i--) {
    int num = 0;
    for (int r = 0; r < ind; r++)
      if (isdigit(a[r][cols[i]])) num = num * 10 + (a[r][cols[i]] - '0');
    if (op == '+') t += num;
    else t *= num;
  }
  cols.clear();
...
void solve() {
  vector<string> a;
  string s;
  while (getline(cin, s) && !s.empty()) a.push_back(s);
  int h = a.size();
  int ind = h - 1, w = 0;
  for (string i : a) w = max(w, (int)i.size());
  for (string &i : a) i.resize(w, ' ');
  ...
  for (int i = 0; i < w; i++) {
    ...
    if (f) ans += calc(a, cols, ind);
    else cols.push_back(i);
  }
  ans += calc(a, cols, ind);
  cout << ans << endl;
}

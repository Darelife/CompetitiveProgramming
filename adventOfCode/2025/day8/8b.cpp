...
void solve() {
  // so, i haven't read the q properly yet....but based on what i read, it felt like a q on mst
  // in which, we have to do kruskal, till like 1000...like, after sorting, just pick the best 1000,
  // that aren't in the same dsu....could be wrong, as i haven't read the q properly.

  // ight, i just saw my input...it's literally a 1000 rows only....so, ig it's just a simple mst
  ...
  for (int i = 0; i<1000; i++) {
    for (int j = i+1; j<1000; j++) {
      int dist = (get<0>(a[i])-get<0>(a[j]))*(get<0>(a[i])-get<0>(a[j]));
      dist += (get<1>(a[i])-get<1>(a[j]))*(get<1>(a[i])-get<1>(a[j]));
      dist += (get<2>(a[i])-get<2>(a[j]))*(get<2>(a[i])-get<2>(a[j]));
      edges.pba({dist, i, j});
    }
  }

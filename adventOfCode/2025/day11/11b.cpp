...
int hashh(const string &s) {
  int ans = 0;
  for (char c : s) ans = ans * 26 + (c - 'a');
  return ans;
}

int dfs(int u, int s, vector<vector<int>>& adj, vector<vector<int>>& ways, int &dac, int &fft, int &out) {
  if (u == out) return (s==3?1:0);
  if (ways[u][s] != -1) return ways[u][s];
  ...
  for (int v: adj[u]) {
    ...
    if (v == dac) ss |= 1;
    if (v == fft) ss |=2;
    ans += dfs(v, ss, adj, ways, dac, fft, out);
  }
  return ways[u][s] = ans;
  // if (ways[u] != -1) return ways[u];
  // int ans = 0;
  // for (int v : adj[u]) ans += dfs(v, adj, ways);
  // return ways[u] = ans;
}


void solve() {
  vector<vector<int>> adj(26*26*26+26);
  int svr = hashh("svr");
  int out = hashh("out");
  int dac = hashh("dac");
  int fft = hashh("fft");
  ...
  for (int i = 0; i < 546; i++) {
    ...
    for (string &name : v) {
        adj[hashh(u)].push_back(hashh(name));
    }
  }


  vector<vector<int>> ways(adj.size(), vector<int>(4,-1));
  // ways[out] = 1;
  int s = 0;
  // cout << dfs(svr, adj, ways) << endl;
  cout << dfs(svr, s, adj, ways, dac, fft, out) << endl;
}

...
void solve() {
  ...
  //   int a = stoll(aa), b = stoll(bb);
  //   x[a] = {max(x[a].first, b), min(x[a].second, b)};
  //   // if (x.find(a) == x.end()) {
  //   //   x[a] = {b, b};  
  //   // } else {
  //   //   x[a] = {max(x[a].first, b), min(x[a].second, b)};
  ...
  //     // ans = max(ans, max())
  //     int t1 = (xx[j]-xx[i]+1)*(abs(x[xx[j]].first-x[xx[i]].second));
  //     int t2 = (xx[j]-xx[i]+1)*(abs(x[xx[j]].second-x[xx[i]].first));
  ...
  for (int i = 0; i < a.size(); i++) {
    for (int j = i + 1; j < a.size(); j++) {
      ...
      if ((x2-x1+1)*(y2-y1+1) <= ans) continue;
      
      int c = 0;
      for (int k = 0; k < a.size(); k++) {
        auto [p1, p2] = a[k];
        auto [q1, q2] = a[(k + 1) % a.size()];
        if (p1 > x1 && p1 < x2 && p2 > y1 && p2 < y2) {
          goto a;
        }
        if (p1 == q1 || p2 == q2) {
          if (p1 == q1 && p1 > x1 && p1 < x2 && min(p2, q2) <= y1 && max(p2, q2) >= y2) goto a;
          if (p2 == q2 && p2 > y1 && p2 < y2 && min(p1, q1) <= x1 && max(p1, q1) >= x2) goto a;
        }
        if (p2 < ((y1+y2)/2.0) == q2 > ((y1+y2)/2.0)) {
          double t = p1+((double)(q1-p1)/(q2-p2))*((y1+y2)/2.0 - p2);
          if ((x1+x2)/2.0 < t) c++;
        }
      }

...
void solve() {
  ...
  for (int i = 0; i < 1176; i++) {
    string s;
    cin >> s;
    int x = stoll(s.substr(0, 2));
    int y = stoll(s.substr(3, 2));
    v.pba({ x, y });
  }
  vector<vint> queries;
  for (int i = 0; i < 197; i++) {
  ...
  for (auto& q : queries) {
    ...
    for (auto& val : v) {
      int v1 = val.first;
      int v2 = val.second;
      auto v1i = find(q.begin(), q.end(), v1);
      auto v2i = find(q.begin(), q.end(), v2);

      if (v1i != q.end() && v2i != q.end()) {
        if (v1i - q.begin() > v2i - q.begin()) {
          is_good = false;
          break;
        }
      }

  ...
  while (getline(cin, line)) {
    for (int i = 4; i < line.size(); i++) {
      ...
      if (line[i - 4] == 'm' && line[i - 3] == 'u' && line[i - 2] == 'l' && line[i - 1] == '(') {
        ...
        // cout << sub << endl;
        // split the sub into 2 parts...separated by a comma
        int comma = sub.find(',');
        string part1 = sub.substr(0, comma);
        string part2 = sub.substr(comma + 1, sub.size() - comma - 1);
        // cout << part1 << " " << part2 << endl;
        int num1 = 0, num2 = 0;
        for (int j = 0; j < part1.size(); j++) {
          num1 += (part1[j] - '0') * pow(10, part1.size() - j - 1);
          // cout << num1 << endl;
        }
...
signed main() {
  ...
  cin.tie(0);
  // take in the input from 1a.txt
  freopen("3a.txt", "r", stdin);

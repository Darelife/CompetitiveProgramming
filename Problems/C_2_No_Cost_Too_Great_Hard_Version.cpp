...
/*
* QUESTZION
* Ight, lets begin....we have 2 arrays, b and a....len = n 
* We can perform this op : a[i]--. Cost += b[i]...here, b[i] = 1 always
* what's the min total cost for there to exist, 2 integers i and j
* gcd(a[i], a[j]) > 1...basically they shouldn't be coprime

* So, we'll have to spend some cost if the elements are all 1 or something liek that
* in that case, we'll make 2 of the 1's, as 2...so cost = 2;
* If all numbers are odd, the worst case answer is 2...making 2 of them even basically
* Yeah, so the answer is always <= 2
* We have to do this all in nlogn. Damn, it could get hard.
* Ight so....If u have even 1 even number, the ans <= 1.
* So basically....just get an even count.
* If it's >= 2, print 0
* If it's 1, then, ans = 0 or 1
* If it's 2, it could be 0,1 or 2
* So, ig we have to check the prime factors pehle se hi to check for common divisors..
* So, a sieve to get all the prime factors. 
* I have an idea....lets, just store the count of all the prime factor's occurances (we'll only
* store once for each prime factor for each a[i]...like,for 4, we'll only store {2}, and not {2,2})
* So, we'll just go through the count then...doneee
* FOr ans = 1, we basically have to keep increasing the number, and checking if it could work or not

* The above was for the hard question....but i feel like this can be solved easily too now...
* nvm, so i just tried making ans = min(ans, b[i]) for 1, and ans = bSorted[0] + bSOrted[1] for 2...but it didn't work
* Cuz we can perform the op multiple times too...and in this case, it could be optimal unlike the easy version

*/

vector<vector<int>> factors(2e5+20);
void solve() {
    int n;
    cin >> n;
    vint a(n), b(n);
    ...
    int ans = INT_MAX;
    for (int i = 0; i<n; i++) {
        for (int j : factors[a[i]]) s.erase(j);
        a[i]++;
        for (int j : factors[a[i]]) {
            int t = s.size();
            s.insert(j);
        ...
        a[i]--;
        for (int j : factors[a[i]]) s.insert(j);
    }
    // cout << 2 << endl;
    sort(b.begin(), b.end());
...
int32_t main() {
  ...
  int t = 1;
  cin >> t;
  for (int i = 2; i<=2e5+15; i++) {
    if (factors[i].size()) continue; // it's composite
    for (int j = i; j<=2e5+15; j+=i) {
        factors[j].pba(i);
    }
  }
  while (t--)

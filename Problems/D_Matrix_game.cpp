...
void solve() {
  ...
  m = (m * (b - 1)) % MOD;
  for (int i = 0; i < a; i++) {
    m = (m * (n - i)) % MOD; // the ncr ka top waala part (the left over part)
  }
  m = (m * modDivide(1LL, factorials[a], MOD)) % MOD;
  // m = (m * factorials[2 * (a - 1) + 1]) % MOD;
  // m = (m * inv[a]) % MOD;
  // m = (m * inv[2 * (a - 1) + 1 - a]) % MOD;
  m = (m + 1) % MOD;
  cout << n << " " << m << endl;
}
...
int32_t main() {
  ...
  // for (int i = 1; i <= 1e6 + 1; i++) {
  //   // inv[i] = modInverse(factorials[i], MOD);
  //   inv[i] = modDivide(1LL, factorials[i], MOD);
  // }

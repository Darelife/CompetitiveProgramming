...
void dfs1(int u, vector<int>& vis, stack<int>& s, vector<vector<int>>& adj) {
  vis[u] = 1;
  for (int v : adj[u]) {
    if (!vis[v]) dfs1(v, vis, s, adj);
  }
  s.push(u);
}

void dfs2(int u, vector<int>& vis, vector<int>& component, vector<vector<int>>& radj) {
  vis[u] = 1;
  component.push_back(u);
  for (int v : radj[u]) {
    if (!vis[v]) dfs2(v, vis, component, radj);
  }
}

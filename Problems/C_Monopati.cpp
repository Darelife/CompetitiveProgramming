...
void solve() {
    ...
    vector<int> maxR(n), maxL(n), minR(n), minL(n);

    minL[0] = a[0][0];
    for (int i = 1; i<n; i++) {
        minL[i] = min(minL[i-1], a[0][i]);
    }

    minR[n-1] = a[1][n-1];
    for (int i = n-2; i>=0; i--) {
        minR[i] = min(minR[i+1], a[1][i]);
    }

    maxL[0] = a[0][0];
    for (int i = 1; i<n; i++) {
        maxL[i] = max(maxL[i-1], a[0][i]);
    }

    maxR[n-1] = a[1][n-1];
    for (int i = n-2; i>=0; i--) {
        maxR[i] = max(maxR[i+1], a[1][i]);
    }
    // int ans = 0;
    set<pair<int,int>> s;
    // int mmin = INT_MAX, mmax = 0;
    for (int i = 0; i<n; i++) {
        int minn = min(minL[i], minR[i]);
        int maxx = max(maxR[i], maxL[i]);
        // ans += minn*(2*n-maxx+1);
        s.insert({minn, maxx});
        // mmin = min(mmin, minn);
        // mmax = max(mmax, maxx);
    ...
    // cout << ans << endl;
    // int ans = mmin*(2*n-mmax+1);
    // for (auto i:s) {
    //     ans += i.first * (2*n-i.second +1);
    // }
    // cout << ans << endl;
    ...
    int mmax = 0, last = 0;
    for (auto i:final) {
        ans += (i.first-last)*(2*n-i.second+1);
        last = i.first;
    }
    cout << ans<< endl;

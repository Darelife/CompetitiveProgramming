...
/*
  * QUESTION
  * We have n rectanges (h[i], and w[i])
  * Now, we have a hs, ws, hb, wb (q such queries)
  * we need the sum of areas of all the rectanges,
  * whose hs < h[i] < hb and ws < w[i] < wb
  *
  * THOUGHT PROCESS
  * Ok wow, i forgot about the q part...i was like, if it's just
  * 1 query, we can do it easily in O(n). However now, we have to do it
  * for each query in log(n) time.
  * Im thinking of taking a prefix sum of the entire thing,
  * and then doing a binary search for the height first, and then, the
  * width, to get the inclusive range...and then, in O(1) get the sum
  * of the areas of the rectangles in the range via the precalculated
  * prefix sum array of the rectange areas.
*/
void solve() {
  int n, q;
  cin >> n >> q;
  vector<int> h(n), w(n);
  int maxx = 1000;
  vector<vector<int>> a(maxx + 2, vector<int>(maxx + 2));
  for (int i = 0; i < n; i++) {
    cin >> h[i] >> w[i];
    a[h[i]][w[i]] += h[i] * w[i];
  }
  vector<vector<int>> pre(maxx + 2, vector<int>(maxx + 2));

...
void solve() {
    ...
    int i = one-1;
    int lar_back = *max_element(a.begin()+one, a.end());
    ...
    vector<int> temp;
    while (i>0) {
        lar_back = max(a[i], lar_back);
        temp.push_back(a[i]);
        if (a[i] < threshold) {
            temp.pop_back();
            ans[threshold].push_back(a[i]);
            for (auto t: temp) {
                ans[a[i]].push_back(t);
            }
            temp.clear();
            threshold = max(threshold, lar_back);
        }
        // debug(ans);
        // for (auto [i,v] : ans) {
        //     cerr << i << endl;
    ...
    if (a[i] > threshold) {
        // cout << i << " " << a[i] << " " << threshold << endl;
        cout << "No" << endl;
        return;
    }
    if (i < one) temp.push_back(a[i]);
    if (a[i] < threshold) {
        // for (int j:temp)
            ans[threshold].push_back(a[i]);
            for (int j:temp) {

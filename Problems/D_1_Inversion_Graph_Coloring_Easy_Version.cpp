...
sequence b
tehre are 2 cols
we can't let a and b have the same colour if a > b and a appears before b
find the number of such subsequences

THINKING PROCESS
so like, we obviously can't have 2 elements > x before x....has to be either 0 or 1
yeah, so this will be dp....I don't see anything else.
dp[0] = till index 0...1 case only
dp[1] = 3 cases...either choose a[0], a[1], or a[1] and a[2]
dp[2]....now, it's tricky....
dp[i] = dp[i-1] + we'll find the indices before i, where a[j] > a[i]. So like, the final ans
could be 2^(i-count of all such indices-1(we have to choose i))

bro...i read the q name now....i'll genuinely be pissed if this turns
out to be a q on graphs....why tf did i spend so much time on dp
i felt so smart that i thought of dp lol

ight, ig i can see how it could be a q on graphs...make edges from
a[i] to a[j] if a[i] > a[j] and i < j
nvm, doing this will also be hard...cuz we can't do it in N^2....
fuckckckckckc
N = 300 ri4uejirfjeskljd2eols

ok, do N^2, and get the edges...then something

also wait, we can do dp too...like, n is just 300....
*/
void solve() {
  int n;
  cin >> n;

...
void solve() {
  ...
  long long power = 1;
  while (power <= r2) { // Stop at r2 to prevent unnecessary computations
    powers.push_back(power);
    if (power > 1e9 / k) break; // Prevent overflow
    power *= k;
  }
  vint kk = powers;
  ...
  for (auto p : kk) {
    ...
    //   }
    // }
    // int lower_x = max(l1, (l2 + t - 1) / t); // Ceil(l2 / p)
    // int upper_x = min(r1, r2 / t);

    // // If range is valid, count the integers
    // if (lower_x <= upper_x) {
    //   ans += (upper_x - lower_x + 1);
    // }

    int lower_x = (l2 + p - 1) / p; // Ceil(l2 / p)
    int upper_x = r2 / p;          // Floor(r2 / p)

    lower_x = max(lower_x, l1);
    upper_x = min(upper_x, r1);

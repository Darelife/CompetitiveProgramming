...
void dfs(int u, int c, vector<int> &col, vector<vector<int>> &adj, vector<int> &cnt, bool &bi) {
  col[u] = c;
...
void solve() {
  ...
  for (int i = 0; i<n;i++) {
    ...
    vector<int> cnt(2, 0);
    bool bi = true;
    dfs(i, 0, col, adj, cnt, bi);
    if (bi) ans += max(cnt[0], cnt[1]);
    // if (ans += max(cnt[0], cnt[1])

...
void solve() {
  ...
  int maxh = 0, maxe = 0, minh = 0, mine = 0;
  for (int i = 0; i < n; i++) {
    maxh += min(a[i][0], a[i][1]);
    maxe += min(a[i][1], a[i][2]);
    minh += min(a[i][0], a[i][1] - min(a[i][1], a[i][2]));
    mine += min(a[i][1] - min(a[i][0], a[i][1]), a[i][2]);
  }
  debug(maxh, maxe);
  int ans = min(maxh, maxe);
  ans = min(ans, (maxh + mine) / 2);
  cout << ans << endl;
}

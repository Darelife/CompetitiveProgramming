...
bool dfs(pair<int, int> u, pair<int, int> p, int n, int m, vector<vector<int>>& a, vector<vector<int>>& vis) {
  int dx[] = { -1, 0, 1, 0 };
  int dy[] = { 0, -1, 0, 1 };
  auto [x, y] = u;
  vis[x][y] = 1;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // if (dfs({ nx, ny }, { x, y }, n, m, a, vis) == false) return false;
    if (dfs({ nx, ny }, { x, y }, n, m, a, vis) == false) ans = false;
  }
  return ans;
}

...
void solve() {
  // to get 1....from left i....s[i] != s[n-i-1]
  // to get 0....from left i....s[i] == s[n-i-1]
  // so basically....go through the binary of the number....and check if it can be a palindrome (we can add leading zeros)
  // so how do we do that??????? 2 pointers...and a flag....if right == 0...and left == 1...then, we keep moving right to left
  // until we find a 1 on right.....then, we just check if it's a palindrome from there or not

  int n;
  cin >> n;

  // okay wait...lets just make it a 32 bit number....we'll have leading zeroes...then, if right == 1 and left == 0 and flag = false...then,
  // move left rightwards until we find a 1...if we find a 1...flag = true...and then, we just check if it's a palindrome from there or not
  int t = n;
  vint b(32, 0);
  int idx = 0;
  while (n > 0) {
  ...
  //       }
  //     }
  //   } else if (flag && b[l] != b[r]) {
  //     cout << "NO" << endl;
  //     return;
  //   } else if (!flag && b[r] != b[l]) {
  //     cout << "NO" << endl;
  //     return;
  //   } else {
  //     l++;
  ...
  // new approach...just get l = first occurance of 1 from left..and r = last occurance of 1 from right
  // then, check if it's a palindrome from l to r
  int l = 0, r = 31;
  while (l < 32 && b[l] == 0)
    l++;
  while (r >= 0 && b[r] == 0)

...
void dfs(int u, int p, vector<int>& a, vector<vector<int>>& adj,
  ...
  for (int v : adj[u]) {
    ...
    dfs(v, u, a, adj, s, dp);
    s[u] += s[v];
    dp[u] += dp[v] + s[v] * (a[u] ^ a[v]);
  }
}

void reroot(int u, int p, int t, vector<int>& ans, vector<int>& a,
  vector<vector<int>>& adj, vector<int>& s, vector<int>& dp) {
  ans[u] = t;
  for (int v : adj[u]) {
    if (v == p) continue;
    reroot(v, u, t - s[v] * (a[u] ^ a[v]) +
      (adj.size() - s[v]) * (a[u] ^ a[v]), ans, a, adj, s, dp);
  }
}
...
void solve() {
  ...
  vector<int> ans(n);
  reroot(0, -1, dp[0], ans, a, adj, s, dp);
  // vpin(s);

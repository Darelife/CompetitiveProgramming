...
void solve() {
  ...
  int ans = 0;
  // if (n > m) {
  //   // cout << (ans1*(n-1)*m) + m*ans2 << endl;
  //   ans = max(ans, )
  // } else {
  //   cout << (ans1*(m-1)*n) + n*ans2 << endl;
  // }
  ans = max(ans, (n-1)*ans1 + (m-1)*ans2);
  ans = max(ans, (n-1)*ans2 + (m-1)*ans1);
  ans = max(ans, (n-1)*ans3 + (m-1)*ans1);
  ans = max(ans, (n-1)*ans1 + (m-1)*ans3);
  ans += (n-1)*(ans1)*(m-1);
  cout << ans << endl;
}

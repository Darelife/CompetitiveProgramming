// SOLVED THIS QUESTION WITH PRAKHARG11, for icpc prep....just submitting from here
...
void solve() {
  ...
  int t = 0;
  int minTime = LLONG_MIN;
  for (int i = 0; i < n; i++) {
    t += a[i];
    minTime = max(minTime, (int)ceil((double)t / (i + 1)));
  }
  cin >> q;
  while (q--) {
    int time;
    ...
    if (time < minTime) {
      ...
      cout << ceil((double)sum / time) << endl;
    }
  }
}

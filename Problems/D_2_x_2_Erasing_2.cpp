...
void solve() {
  ...
  for (int i = 0; i<n-2; i++) {
    for (int j = 0; j<m-1; j++) {
      if (g[i][j] == '#' && g[i][j+1] == '#' && g[i+1][j] == '#' && g[i+1][j+1] == '#' && g[i+2][j] == '#' && g[i+2][j+1]) {
        b++;
      }
    }
  }
  for (int i = 0; i<n-1; i++) {
    for (int j = 0; j<m-2; j++) {
      if (g[i][j] == '#' && g[i][j+1] == '#' && g[i+1][j] == '#' && g[i+1][j+1] == '#' && g[i][j+2] == '#' && g[i+1][j+2]) {
        b++;
      }
    }
  }
  for (int i = 0; i<n-2; i++) {
    for (int j = 0; j<m-2; j++) {
      if (g[i][j] == '#' && g[i][j+1] == '#' && g[i+1][j] == '#' && g[i+1][j+1] == '#' && g[i][j+2] == '#' && g[i+1][j+2] && g[i+2][j] == '#' && g[i+2][j+1] == '#' && g[i+2][j+2] == '#') {
        c++;
      }
    }
  }

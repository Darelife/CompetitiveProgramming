...
void darelife() {
  ...
  // for (int i = 0; i<k-1; i++) pq.push(a[i]);
  // for (int i = n-k+1; i<n; i++) pq.push(a[i]);

  // need to remove n-k+1 elements. we've removed n-k - (k-1) + 1 elements
  // = n-2k+2 elements....need to remove n - k +1 - n + 2k-2 elements
  // = k - 1 elements
  ...
  if (2*k <= n) {
    ...
    for (int i = 0; i<k-1; i++) ans += max(a[i], a[n-i-1]);
  } else {
    for (int i = 0; i<n-k+1; i++) {
      ans += max(a[n-k-i], a[i+k-1]);
    }
  }

...
void solve() {
  ...
  // ight, so this sort of feels obvious, we just need to equalize the count of 1s on both sides, to get the max. 
  // and if it's odd, then, a = b-1 or vice verse...imma just check the count once
  ...
  // now im not sure if the approach should change for odd / even 1s....
  // lets just find the halfway in the vertical direction, and then, the horizontal. 
  // like, first we can keep going down until we find the vertial mid, and then, we'll keep going right, until we find the horizontal mid
  // then, we'll go down once again, and then right till we reach the end of the row....and then, down till we reach the end point

  int ans = (cnt1/2)*(cnt1-cnt1/2);
  cout << ans << endl;

  // ohh fuck, we have a limit to the number of D's and R's.....im an idiot...it doesn't matter...we have to anyways to n R's, and m D's....
  ...
  // int rmid = -1;
  // int totr = 0;
  // int req = cnt1/2 - tot + cnt[cmid];
  // if (req) {
  //   // for (int i = m-1; i>=0; i--) {
  //   for (int i = 0; i<m; i++) {
  //     totr += (a[cmid][i] == 1);
  //     // if (totr + tot - cnt[cmid] >= cnt1/2) {
  //     //   rmid = i;
  //     //   break;
  //     // }
  //     if (totr == req) {
  ...
  // debug(cmid, rmid);
  // for (int i = 0; i < cmid+1; i++) cout << 'D';
  // for (int j = 0; j < rmid; j++) cout << 'R';
  // if (cmid < n) cout << 'D';
  // for (int j = rmid; j < m; j++) cout << 'R';  
  // for (int i = cmid + 2; i < n; i++) cout << 'D';
  // cout << endl;
}

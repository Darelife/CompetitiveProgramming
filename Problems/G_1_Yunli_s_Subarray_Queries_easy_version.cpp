...
void solve() {
    ...
    // apparently, it doesn't return null by default or something...
    for (int i = 0; i<n; i++) lar.insert(0);
    for (int i = 0; i<k-1; i++) {
        lar.erase(lar.find(offset[a[i]-i]));
        offset[a[i]-i]++;
        lar.insert(offset[a[i]-i]);

    }
    for (int i = k-1; i<n; i++) {
        // ans[{i-(k-1), i}]
        // offset[a[i]-i]++;
        // // auto it = offset.rbegin();
        // // ans[{i-(k-1), i}] = it->first + i;
        // // }
        lar.erase(lar.find(offset[a[i]-i]));
        offset[a[i]-i]++;
        lar.insert(offset[a[i]-i]);
        ans[{i-(k-1), i}] = k-*lar.rbegin();
        lar.erase(lar.find(offset[a[i-(k-1)]-i+k-1]));
        offset[a[i-(k-1)]-i+k-1]--;
        lar.insert(offset[a[i-(k-1)]-i+k-1]);

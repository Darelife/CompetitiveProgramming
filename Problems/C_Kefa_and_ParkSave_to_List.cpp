...
void dfs(int curr, vector<vint>& edges, int currPar, int temp, int& ans, int& m, vint& c) {
  if (edges[curr].size() == 1 && curr != 0) { // leaf 
    if (c[curr]) temp++;
    if (temp <= m) ans++;
    return;
  }
  ...
  for (auto child : edges[curr]) {
    if (child != currPar) {
      dfs(child, edges, curr, temp, ans, m, c);
    }
  }
  return;
}

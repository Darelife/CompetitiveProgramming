...
double dp(int i, int j, int k) {
  ...
  if (vis[i][j][k]) return memo[i][j][k];

  vis[i][j][k] = true;
  double ans = 0.0;
  double s = i + j + k;

  if (i > 0) ans += (i / s) * dp(i - 1, j, k);
  if (j > 0) ans += (j / s) * dp(i + 1, j - 1, k);
  if (k > 0) ans += (k / s) * dp(i, j + 1, k - 1);
  ans += (double)n / s;
...
void solve() {
  ...
  vector<int> cnt(4, 0);
  for (int x : a) cnt[x]++;
  memo = vector<vector<vector<double>>>(n + 1, vector<vector<double>>(n + 1, vector<double>(n + 1, 0.0)));
  vis = vector<vector<vector<bool>>>(n + 1, vector<vector<bool>>(n + 1, vector<bool>(n + 1, false)));

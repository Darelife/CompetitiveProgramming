...
void dfs(int u, const vector<pair<int, int>>& adj, vector<int>& dist, string& s) {
  if (u == -1) return;
  if (adj[u].first == -1 && adj[u].second == -1) return;

  auto [v1, v2] = adj[u];
  dfs(v1, adj, dist, s);
  dfs(v2, adj, dist, s);
  if (v1 == -1) {
    dist[u] = (s[u] == 'U' || s[u] == 'L') + dist[v2];
  } else if (v2 == -1) {
    dist[u] = (s[u] == 'U' || s[u] == 'R') + dist[v1];
  } else {
    dist[u] = min((s[u] == 'U' || s[u] == 'R') + dist[v1], (s[u] == 'U' || s[u] == 'L') + dist[v2]);
  }
}

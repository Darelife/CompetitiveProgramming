...
/*
THINKING PROCESSS
I can clearly see that we just have to make n-1 combinations from each edge...if x > y, then, we'll say
that P[u] > p[v], otherwise, P[v] > P[u]
So, once we have a set of those, we can easily make the perm
Now the point is implementing it...okay, i can see a toposort thing...that we could do heree
something like, making the edges directed..based on the values of x and y
*/
void solve() {
  int n;
  cin >> n;

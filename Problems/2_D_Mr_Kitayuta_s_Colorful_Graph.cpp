...
void solve() {
    int n, m;
    cin >> n >> m;
    // so, im thinking of a dsu....but in this, we need unique colours along with ensuring that the elements are connected.
    // so, lets traverse each node, .....ok like, lets make a dsu for each colour

    vector<DSU> dsu(101, DSU(n));
    ...
    while (q--) {
        ...
        for (int color = 1; color <= 100; color++) {
            if (dsu[color].find(a) == dsu[color].find(b)) {
                ans++;
            }
        }
        cout << ans << endl;

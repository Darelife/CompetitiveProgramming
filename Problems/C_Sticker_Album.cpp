...
void solve() {
  int n, l, r;
  cin >> n >> l >> r;
  bool isZero = (l == 0) ? true : false;
  vector<double> dp(n + 1, 0), suffix(n + 2, 0);
  for (int i = n - 1; i >= 0; i--) {
    int x1 = i + l, x2 = i + r;
    x2 = min(n, x2);
    if (x1 >= n) {
    ...
    if (isZero) {
      ...
      // dp[i] /= (1 - 1 / (r - l + 1));
      x2 = min(n - 1, x2);
      double range = suffix[x1 + 1] - suffix[x2 + 1];
      dp[i] = (r - l + 1 + range) / (r - l);
      suffix[i] = dp[i] + suffix[i + 1];
      continue;
    }
    double range = suffix[x1] - suffix[x2 + 1];
    // dp[i] = 1.0 + range / (double)(r - l + 1) + isZero * double(r - l);
    dp[i] = 1.0 + range / (double)(r - l + 1);
    suffix[i] = dp[i] + suffix[i + 1];
  }

...
void solve() {
  ...
  int f0 = 0, f1 = 0, type = 0;
  for (char i : s) {
    if (i == 'R') {
      f0 = 1;
      continue;
    }
    if (f1 && isdigit(i)) continue;
    if (f0 && isdigit(i)) f1 = 1;
    if (f1 && i == 'C') type = 1;
  }
  // if (type == 1 && s[s.size()-1] == 'C') type = 0;
  
  if (type == 0) {
    string t = "";
    while (s[t.size()] <= 'Z' && s[t.size()] >= 'A') {
      t += s[t.size()];
    }
    int row = stoi(s.substr(t.size()));
    int c = 0;
    for (char i:t) {
      c = c*26 + 1 + i - 'A';
    }
    ...
    string rr = s.substr(1, i-1); 
    string cc = s.substr(i+1); 
    int r = stoi(rr); 
    int c = stoi(cc); 
    string ans = ""; 
    while (c) { 
      c--; ans += (char)('A' + c%26); c /= 26; 
    } 

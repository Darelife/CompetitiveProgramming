...
void solve() {
    ...
    while (l <= r) {
        ...
        // int cnt = 0;
        set<int> temp;
        for (int j = 0; j<=a[0]-mid && temp.size()<k; j++) {
            temp.insert(j);
        }
        for (int j = a[n-1]+mid; temp.size()<k && j<=x; j++) {
            temp.insert(j);
        }
        for (int i = 0; i < n - 1; i++) {
            if (a[i + 1] - a[i] >= 2 * mid) {
                ...
                // last = a[i];
                // cnt++;
                for (int j = a[i] + mid; j <= a[i + 1]-mid && temp.size()<k; j++) {
                    temp.insert(j);
                    // cnt++;
                }
            }
        ...
        debug(mid);
        if (temp.size()>= k) {
            ans = vector<int>(temp.begin(), temp.end());
            l = mid + 1;
            t = mid;
        } else {
            r = mid - 1;
    ...
    // priority_queue<pair<int,int>> pq; //diff, loc
    // pq.push({a[0], 0});
    // pq.push({x - a[n-1], x});
    // for (int i = 0; i<n-1; i++) {
    //     int diff = a[i+1]- a[i];
    //     if (diff%2==0) {
    //       pq.push({diff/2, a[i]+diff/2});
    //     } else {
    //       pq.push({diff/2, a[i]+diff/2+1});
    //       pq.push({diff/2, a[i+1]-diff/2});
    //     }
    // }

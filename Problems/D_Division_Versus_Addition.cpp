...
vector<int> ans(250010);
vector<int> two(250010);
...
void solve() {
  ...
  p[0] = { ans[a[0]], two[a[0]] };
  for (int i = 1; i < n; i++) {
    p[i] = { p[i - 1].first + ans[a[i]], p[i - 1].second + two[a[i]] };
  }
  ...
  for (int i = 0; i < q; i++) {
    ...
    int t = (l > 1) ? p[l - 2].first : 0;
    int tt = (l > 1) ? p[l - 2].second : 0;
    cout << p[r - 1].first - t + (p[r - 1].second - tt) / 2 << endl;
  }
}
...
void pre() {
  ...
  for (int i = 3; i < 250010; i++) {
    // 100000 & 011111

    // 10001 & 10000 =  10000
    // 10000 & 01111 = 0
    if (i > 1 && __builtin_popcount(i - 1) == 1) {
      two[i] = 1;
    }
  }
}

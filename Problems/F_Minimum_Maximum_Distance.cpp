...
/*
  * QUESTION
  * We have a tree, with some marked nodes, and f(i) = dist between i and the farthest marked node
  * Find the min value of f(i) over all the i's
  *
  * THOUGHT PROCESS
  * i mean, i can directly see a diameter based thing here. Find the diameter, and then, ans = ciel(diameter/2)
*/
...
void solve() {
  ...
  dist2[u] = 0;
  dfs(u, -1, dist2, adj);
  // int d = *max_element(dist2.begin(), dist2.end());
  int d = -1;
  for (int i : marked) {
    if (dist2[i - 1] > d) d = dist2[i - 1];
  }
  cout << (d + 1) / 2 << endl;
}

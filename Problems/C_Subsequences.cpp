...
struct segTree {
  ...
  void segBuild(int start, int end, int index) {
    if (start == end) {
  ...
  void segUpdate(int start, int end, int index, int queryIndex, Update& u) {
    if (start == end) {
      u.apply(tree[index]);
      return;
    }
    int mid = start + (end - start) / 2;
    if (mid >= queryIndex) {
      segUpdate(start, mid, 2 * index, queryIndex, u);
    } else {
      segUpdate(mid + 1, end, 2 * index + 1, queryIndex, u);
    }
    tree[index].merge(tree[2 * index], tree[2 * index + 1]);
  }
  Node segQuery(int start, int end, int index, int l, int r) {
    if (start > r || end < l) {
      return Node();
    }
    if (start >= l && end <= r) {
    ...
    int mid = start + (end - start) / 2;
    Node left, right, ans;
    left = segQuery(start, mid, 2 * index, l, r);
    right = segQuery(mid + 1, end, 2 * index + 1, l, r);
    ans.merge(left, right);
    return ans;
  }
  void make_update(int index, int val) {
    Update newUpdate = Update(val);
    segUpdate(0, n - 1, 1, index, newUpdate);
  }
  Node make_query(int l, int r) {
    return segQuery(0, n - 1, 1, l, r);
  }
};
...
void solve() {
  ...
  vcin(a, n);
  vint b(n);
  vector<segTree> sgt(k + 2, segTree(n, b));
  for (int i = 0; i < n; i++) {
    sgt[1].make_update(a[i] - 1, 1);
    for (int j = 2; j <= k + 1; j++) {
      int val = sgt[j - 1].make_query(0, a[i] - 2).val;
      sgt[j].make_update(a[i] - 1, val);
    }
  }
  cout << sgt[k + 1].make_query(0, n - 1).val << endl;
}

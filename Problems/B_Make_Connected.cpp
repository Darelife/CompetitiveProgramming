...
void solve() {
    //  can see after drawing it ki like, (x,y)'s max connection in my drawing is (x+7, y+8).....so something like that for all directions
    // gives us that we can connect it if pairwise, if (abs((x1-y1)-(x2-y2)) <= 1) then it can work
    // oh wait, in the other orientation, it's x+y too, so we can just do abs((x1+y1)-(x2+y2)) <= 1 too
    int n;
    cin >> n;
    ...
    // wait, instead of doing it paitwise, i can just do it overall....like, the max and min of (x+y) and (x-y) for all the B's
    ...
    //  else {
    //     debug(min1,min2,max1,max2);
    //     cout << "NO" << endl;
    // }
    // failed the case: where n = 3....nvm, basically, ig i need to check if all the points are literally connected with each other or not...pehle se hi....like.....fuckckck....the 2x2 case is a special case...
    // basically there can only be a 2x2 square and no other B's...to get a yes

...
/*
* QUESTION:
* There are N balls, and M baskets. We'll put each new ball in the basket
* with the least number of balls. If there are several such baskets,
* we'll choose the one closest to the middle (abs((m+1)/2-i(1 based))
* is min). If we still have several such baskets, we'll choose the left one.
* For each ball, we must print the basket number.
*/
void solve() {
  int n, m;
  cin >> n >> m;
  ...
  if (m % 2) {
    ...
    // int cnt = 1;
    // while (cnt < n) {
    //   for (int i = m / 2 - 1; i >= 0 && cnt < n; i--) {
    //     cout << i + 1 << endl << m - i << endl;
    //     cnt += 2;
    //   }
    // }
    for (int i = 0; i < m / 2; i++) {

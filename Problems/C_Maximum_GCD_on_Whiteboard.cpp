...
void solve() {
    // ik that it's nlogn...but it's sort og brute onli.
    // like, im thinking of just abusing the fact that a[i] <= n
    ...
    for (int i = n; i >= 1; i--) {
        int c = 0;
        if (min(n, 4*i - 1) > min(n, 3*i)) {
            c = pre[min(n, 4*i - 1)] - pre[min(n, 3*i)];
        }

        if ( pre[min(n, 3 * i - 1)]-(2*i <=n?cnt[2*i]:0)-cnt[i] + c <= k) {
            cout << i << endl;
            return;
        }
    }

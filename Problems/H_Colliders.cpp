...
void solve() {
  ...
  // We will store the elements that are on, and also a map, which is store conflicts
  set<int> on;
  // conflicts = {factor, element corresponding to that factor in the set}
  ...
  forr(i, m) {
    ...
    cin >> a >> b;
    if (a == '-') {
      // just remove the element from the set if it's in the set -> Success
      // if it's not in the set, "Already Off"
      // also remove the conflicts
      if (on.find(b) == on.end()) {
      ...
      // first of all, if it's on, "Already On"
      // Otherwise, time to check for conflicts
      if (on.count(b)) {
        cout << "Already on" << endl;
      } else {
        int success = 1;
        for (auto x : sieve[b]) {
          // now we have the factors of b -> check if any of them are in it already
          if (conflicts.find(x) != conflicts.end()) {
            success = 0;
            cout << "Conflict with " << conflicts[x] << endl;
            break;
          }
        }
        if (success) {
          on.insert(b);
          // put all the factors of b in the conflics map
          for (auto x : sieve[b]) {
            conflicts[x] = b;
          }
          cout << "Success" << endl;

/*
  * Problem 1: Beautiful Numbers
  * Statement: Given a number N , find the smallest beautiful number strictly greater than N .
  * A beautiful number is defined as a number in which every digit d occurs exactly d  times.
  * Examples:      22  , 212, 32233 are beautiful numbers.
  * 99 , 12      are not beautiful.
  * Constraints:      1≤N≤1012  0≤d≤9
*/
...
void solve() {
  ...
  while (q--) {
    ...
    cin >> n >> k;
    s.insert({ n, idx++ });
    cout << (*s.find_by_order(s.size() - k)).first << '\n';
  }
}

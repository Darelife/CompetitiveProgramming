...
void solve() {
  // things to note
  /*
    * 1. We can only use a domino once.
    * 2. Now, if n <= 6, the answer will simply be m,
    * because we can place all the dominoes...for each type of edge,
    * we can assign their very own number...for 1, we can assign 1, and so on
    *
    * 3. if n > 6. Now this is the main part.
    * basically n = 7, as n <= 7...
    * the max value of m = 7*6/2 = 21
    * loop around, and select 2 vertices, and assign them the same value.
    * Now, just assume those 2 vertices to be the same when u iterate
    * through all the edges, and put them in a set...ig it should work out.
  */
  int n, m;
  cin >> n >> m;

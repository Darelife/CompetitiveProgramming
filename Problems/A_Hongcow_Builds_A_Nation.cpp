...
public:
  ...
  void union_sets(int a, int b) {
    ...
    if (a != b) {
      ...
      parent[b] = a;
      size[a] += size[b];
      edge_count[a] += edge_count[b] + 1; // add existing edges + 1 new edge
    } else {
      // Adding an edge within the same component
      edge_count[a]++;
    }
  }
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    ans += 1LL * dsu.size[d] * (dsu.size[d] - 1) / 2;
    largest = max(largest, dsu.size[d]);
    leftover -= dsu.size[d];
  }

  ans -= 1LL * largest * (largest - 1) / 2;
  ans += 1LL * (largest + leftover) * (largest + leftover - 1) / 2;
  ans -= m;

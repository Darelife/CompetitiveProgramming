...
void solve() {
  ...
  // * now, lets go through the question
  // * so, a_i * a-J has to be <= max j - min i = n-1 - 0 = n-1
  // * so, a_i * a_j <= n-1...sort of like a square root (so, i knew this already about the q
  // * as, i had discussed this with a friend)....didn't do anything after this
  // * we can maybe make a vector of pairs (a_i, i), and then sort
  // * yeah so like, maybe....we could then, iterate over the vector...(square waala loop)
  // * but break it as soon as a_i * a_j > n-1.

  // * hmm, so ig the break condition needs to be somehow improved...like, i can see it running N^2
  // * like, if the numbers are small.

  // * a_i * a_j + i == j < n...1 based hona chaiye na waise? somehow tc 7 tak kaise pohunch gaya?
  // * im lost ab...i've no clue what to do now. I tried thinking of moving i to the LHS, but i don't see how it can make any difference
  // * probably cuz my array is filled with 1s
  ...
  // for (int i = 1; i<n && a[i] == 1; i++) {
    //   if (b[i].second - b[i-1].second == 1) ans++;
    // }
    
    // for (int i = 0; i<n; i++) {
      // for (int j = i+1; j<n; j++) {
        //   if (a[j] == 1) continue;
        //   if (b[i].first *b[j].first > n-1) {
          //     // debug(i, j, n);
          //     break;
          //   }
          //   if (b[i].first *b[j].first == abs(b[j].second-b[i].second)) 
          //     ans++;
          // }
  int ans =0;
  for (int i = 0; i<n; i++) {
    for (int j = 1; a[i] * j <= n-1; j++) {

      // * yaha, j is the a[j]
      // * a[j] * a[i] +i = j
      // * smallest value of j = a[i]*a[i] + i///nvm, it's not sorted
      if (a[i]*j+i >= n) break;
      if (a[i]*j+i < n && a[a[i]*j + i] == j) ans++; 
    }
  }

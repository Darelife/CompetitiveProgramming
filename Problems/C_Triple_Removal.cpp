...
void solve() {
  ...
  vector<tuple<int, int, int>> tripp(n);
  tripp[0] = { a[0] == 0, a[0] == 1, 0 };
  for (int i = 1; i < n; i++) {
    // cnt1 += (a[i] == 1);
    // cnt0 += (a[i] == 0);
    tripp[i] = { get<0>(tripp[i - 1]) + (a[i] == 0), get<1>(tripp[i - 1]) + (a[i] == 1), get<2>(tripp[i - 1]) + (a[i - 1] != a[i]) };
  }
  // for (int i = 0; i < n; i++) {
  //   cout << get<2>(tripp[i]) << " ";
  // }
  ...
  for (int qq = 0; qq < q; qq++) {
    int l, r;
    cin >> l >> r;
    int z = get<0>(tripp[r - 1]) - (l > 1 ? get<0>(tripp[l - 2]) : 0);
    int o = get<1>(tripp[r - 1]) - (l > 1 ? get<1>(tripp[l - 2]) : 0);
    int s = get<2>(tripp[r - 1]) - (l > 1 ? get<2>(tripp[l - 1]) : 0);
    if (z % 3 || o % 3) {
      cout << -1 << endl;
      continue;
    }

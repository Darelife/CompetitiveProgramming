...
/*
* Question
* We have n swords, and have m monsters
* damage[i] = a[i]
* health[i] = b[i]
* Sword x can kill monster m, if x >= b[m]
* After killing monster i with a sword of damange x, if C[i] <= 0, we'll lose the sword.
* Otherwise, Sword's damange = max(C[i], x)

* Wow, my brain's already at max capacity after reading the question!!!
* OK, it's just the C[i] stuff. 
* If we kill them all in a sorted order???
* We have to take care of the C[i]...if we only kill the monsters, if c[i] is not 0??
* If we somehow, try to find the perfect damage, so that after all the c[i]'s, we'll kill the most monsters....
* Not exactly convinced, but binary search for this??
* Okay, i didn't read what we had to find lmfao....i assumed that we had to somehow simulate something...idk what i was thinking
* So, we have to find the max monsters we can kill....something like, a graph, or greedy something...thought of graph, cuz i wanted some sort of a route
* to kill which monster in an order, to keep increasing the damage and all.

* Lets go through tc 3
* swords : 1, 7, 7
* monsters : {6, 2}, {6, 0}, {2, 0}, {2, 7}, {2, 2}
* When we do c[i] = 0, we're basically getting rid of a sword. So, that will be the last operation for any sword. 
* Lets sort all the monsters, (ascending of monster health, and descending of c)
* {2, 7}, {2, 2}, {2, 0}, {6, 7}, {6, 0}
* Lets ignore the c[i] = 0 for now. We can use the sword with damage 7, and kill {2, 7}, {2, 2}, {6, 7}, {the most health monster, with health <= damage}....
* Yeah that's it...lets just run this thing...actually we don't need to sort in descending of c. 
* Actually, lets also optimize the sword selection. We'll just select the lowerbound.
*/
void solve() {
  int n, m;
  cin >> n >> m;
  ...
  for (auto [bb, cc]:mons) {
    if (cc == 0) continue;
    auto spointer =mul.lower_bound(bb);
    if (spointer == mul.end()) {
      break;
    }
    int sword = *spointer;
    ...
    mul.insert(max(sword, cc));
  }
  // cout << ans << " ";

  for (auto [bb, cc] : zero) {
    auto spointer = mul.lower_bound(bb);
    if (spointer == mul.end()) break;

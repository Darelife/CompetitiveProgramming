...
void solve() {
  ...
  // int larInd = max_element(a.begin(), a.end()) - a.begin();
  // int l = larInd, r = larInd;
  // int cnt = 1;
  // while (cnt != n) {
  //   int left = (l - 1 >= 0) ? a[l - 1] : -1;
  //   int right = (r + 1 <= n - 1) ? a[r + 1] : -1;
  //   int d = 0;
  //   if (left == lar) {
  //     l--; lar = left; d = 1;
  //   } else if (right == lar) {
  ...
  int left = b[n], right = b[n];
  for (int i = n - 1; i > 0; i--) {
    int j = b[i];
    if (j < left && j == left - 1) left = j;
    else if (j > right && j == right + 1) right = j;
    else {
      cout << "NO" << endl;
      return;
    }

...
void solve() {
  ...
  // bro, this question sucks...it's 3:02 here,and i've finally understood the testcases now...actually i've finally understood the q ab

  // im blank now...ok nvm....yeah, lets go through the factors
  ...
  for (int i = 1; i*i <= n; i++) {
    if (n%i == 0) {
      int tt = n/i;
      // vector<set<int>> b(n/i);
      // for (int j = 0)
      // ok, i need to somehow figure out that checking part. Like, how to see if there's something in a bunch of them...for each char, we'll have to store the indices ig
      // orrrrrrr.....uhhh, foejme,f lite, i can't think....fuck nvm...i can....i can just store everything in the set....and then traverse it
      // but, i can't do set traversal cuz, i'll have to do sqrt(N) * N * K * log(N/K).....uhhh so i just did the calculation...it's 3*1e8....ig 4*1e8 is the limit, but thats the asm instruction limit right?
      // fuck it, imma do it....i'll just optimize this factor thing a bit then....
      fac.pba(i);
      if (i != tt) {
        fac.pba(tt);
      }
  ...
  for (int i:fac) {
    ...
    for (int j = 0; j<i; j++) {
      ...
      for (int k = 0; k<26; k++) {
        ...
        for (int l = j;l < n; l+=i) {
          if (s[l].find(c) == s[l].end()) {
            f1 = 0;
            goto b;
          }
        }
        // fuck, i should have used flags...now im confused af
        t += c;
        f = 1;
        goto c;
        b:

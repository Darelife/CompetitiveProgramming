...
class Eraser1D(Scene):
    def construct(self):
        ...
        # Create title
        title = Text("1D Eraser Problem", font_size=36).to_edge(UP)
        ...
        i = 0
        ans = 0
        counter = Text("Operations: 0", font_size=36).to_edge(DOWN)
        self.play(Write(counter))
        
        # Add rule explanation
        rule = Text("Rule: Only erase when encountering a BLACK cell", 
               font_size=24, color=RED).next_to(counter, UP, buff=0.5)
        ...
        while i < n:
            ...
            if s[i] == 'B':
                ...
                action_text = Text("Found BLACK cell! Erasing...", font_size=28, color=YELLOW)
                action_text.next_to(boxes, DOWN, buff=0.8)
                self.play(Write(action_text))
                
                # Increment answer counter
                ans += 1
                new_counter = Text(f"Operations: {ans}", font_size=36).to_edge(DOWN)
                ...
                if i + k < n:
                    skip_text = Text(f"Skip k cells forward...", font_size=28, color=GREEN)
                    skip_text.next_to(boxes, DOWN, buff=0.8)
                ...
                action_text = Text("Found WHITE cell, moving to next...", font_size=28, color=BLUE)
                action_text.next_to(boxes, DOWN, buff=0.8)
                self.play(Write(action_text))
                self.wait(0.3)
                self.play(FadeOut(action_text))
        ...
        result_text = Text(f"Total operations needed: {ans}", font_size=36, color=GREEN)
        result_text.next_to(boxes, DOWN, buff=1.5)

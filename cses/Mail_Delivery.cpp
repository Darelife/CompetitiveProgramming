...
void dfs(int u, vector<set<int>>& adj, vector<int>& vis, vector<int>& path) {
  while (!adj[u].empty()) {
    int v = *adj[u].begin();
    // We basically are going to erase this edge from both v, and u's adj's thing
...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    // like, u have to be able to exit every node too
    if (deg[i] % 2) {

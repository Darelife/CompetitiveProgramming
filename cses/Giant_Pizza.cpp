...
void dfs1(int u, vector<vint>& adj, vector<bool>& visited, stack<int>& st) {
  visited[u] = true;
  for (int v : adj[u])
...
void dfs2(int u, vector<vint>& rev, vector<int>& comp, int cid) {
  comp[u] = cid;
  for (int v : rev[u])
    if (comp[v] == -1) dfs2(v, rev, comp, cid);
}
...
void solve() {
  ...
  vector<vint> adj(2 * m), rev(2 * m);

  for (int i = 0; i < n; i++) {
    string a_sign, b_sign;
    int a, b;
    ...
    int a_var = 2 * a + (asign ? 1 : 0);
    int b_var = 2 * b + (bsign ? 1 : 0);

    int not_a = a_var ^ 1;
    int not_b = b_var ^ 1;

    adj[not_a].push_back(b_var);
    adj[not_b].push_back(a_var);

    rev[b_var].push_back(not_a);
    rev[a_var].push_back(not_b);
  }
